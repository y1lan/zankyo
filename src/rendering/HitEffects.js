...
export class HitEffects {
  ...
  spawn(position, quality = 'perfect') {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const target = position.clone().add(dir.multiplyScalar(3 + Math.random() * 8));
    ...
    for (let r = 0; r < 3; r++) {
      const rr = 1.5 + r * 0.8;
      const ringGeo = new THREE.TorusGeometry(rr, 0.04, 8, 48 + r * 16);
      const ringMat = new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.9 - r * 0.25 });
  ...
  _secondary(pos, color, count) {
    for (let i = 0; i < count; i++) {
      ...
      const target = pos.clone().add(dir.multiplyScalar(1.5 + Math.random() * 3));
